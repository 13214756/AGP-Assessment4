...
APickup::APickup()
{
	...
	PickupBoundingBox->SetWorldScale3D(FVector(2.0f, 2.0f, 2.0f));


	//LootBox = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("Loot Box"));
	//LootBox->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepWorldTransform);
	/*LootBoundingBox = CreateDefaultSubobject<UBoxComponent>(TEXT("Loot Bounding Box"));
	LootBoundingBox->AttachToComponent(LootBox, FAttachmentTransformRules::KeepWorldTransform);
	LootBoundingBox->SetGenerateOverlapEvents(true);
	LootBoundingBox->OnComponentBeginOverlap.AddDynamic(this, &APickup::OnLootBeginOverlap);
	LootBoundingBox->SetWorldScale3D(FVector(2.0f, 2.0f, 2.0f));*/
	//bBoxClosed = true;
}
...
/*
void APickup::OnExitPickup(class UPrimitiveComponent* OverlappedComponent, class AActor* OtherActor, class UPrimitiveComponent* OtherComponent, int32 OtherBodyIndex)
{
...
/*
void OnLootBeginOverlap(class AActor* OverlapActor)
{
